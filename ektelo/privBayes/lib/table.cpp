...
table::table(const int* input, const string& config, bool func1, int m, int n) : func(func1) {
    //read domain
    istringstream sdomain(config);

    string s;
    while (getline(sdomain, s)) {
        if (s[0] == 'D') 
            ...
            //internal parameter: cut into at most 2^4 blocks
            translators.push_back(make_shared<ctranslator>(min, max, 5));       
        }
    }
...
int table::getSize(int col) {       // gets the domain size (hopefully)
    return translators[col]->size();
    // return 1;
}
...
// void table::printo_csv(const string& filename) {
//  ofstream fsvm(filename);
//  for (const auto& tuple : data){
//      string output;
//      int index = 1;
